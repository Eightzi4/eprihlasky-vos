...
@section('content')
    @php
        $descriptionLabels = [
            ...
            \App\Models\AuditLog::DESCRIPTION_EXPORT_APPLICATION_ZIP => 'Export přihlášky do ZIP',
            \App\Models\AuditLog::DESCRIPTION_BULK_EXPORT_CSV => 'Hromadný export CSV',
            \App\Models\AuditLog::DESCRIPTION_BULK_EXPORT_PDF => 'Hromadný export PDF',
            \App\Models\AuditLog::DESCRIPTION_BULK_EXPORT_ZIP => 'Hromadný export ZIP',
            \App\Models\AuditLog::DESCRIPTION_EXPORT_AUDIT_LOG => 'Export auditního logu',
            ...
            \App\Models\AuditLog::DESCRIPTION_MOVE_TO_FURTHER_ROUND => 'Přesun do dalšího kola',
        ];
    @endphp

    <div
        class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-sm border border-white/60 ring-1 ring-black/5 overflow-hidden">
        <div class="px-8 py-5 border-b border-gray-100/80 bg-white/40 flex items-center gap-3 flex-wrap">
            ...
            <div class="flex items-center gap-3" x-data="{ q: '{{ request('search') }}', loading: false }">
                <form method="GET" action="{{ route('admin.audit-logs') }}" class="relative" x-init="$watch('q', () => { loading = true; const url = new URL('{{ route('admin.audit-logs') }}', window.location.origin);
                    url.searchParams.set('search', q);
                    window.history.replaceState({}, '', url);
                    fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } }).then(r => r.text()).then(html => { const parser = new DOMParser(); const doc = parser.parseFromString(html, 'text/html'); const newContent = doc.querySelector('#audit-table-wrapper'); if (newContent) document.getElementById('audit-table-wrapper').innerHTML = newContent.innerHTML;
                        loading = false; }); })">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <span class="material-symbols-rounded text-gray-400 text-[18px]">search</span>
                    </div>
                    <input type="text" name="search" x-model="q" placeholder="Jméno, e-mail, IP, popis..."
                        class="block w-64 pl-9 pr-3 py-2 border border-gray-200 rounded-xl bg-white/50 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-school-primary focus:border-school-primary">
                </form>
                <x-button as="a" href="{{ route('admin.audit-logs.export', request()->query()) }}"
                    text="Export CSV" icon="table_view" variant="secondary" size="sm" spanClass="text-gray-700" />
            </div>
        </div>
        ...
        <div id="audit-table-wrapper">
                        ...
                        @forelse ($logs as $log)
                            <tr class="hover:bg-red-50/20 transition-colors">
                                <td class="px-5 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div class="font-semibold text-gray-900">{{ $log->created_at?->format('j. n. Y') }}
                                    </div>
                                ...
                                <td class="px-5 py-4 whitespace-nowrap text-sm text-gray-500">
                                    ...
                                    <div class="text-xs text-gray-400">do
                                        {{ $log->not_after?->format('j. n. Y H:i') ?? '—' }}
                                    </div>
                                </td>
                            </tr>
                        @empty
            ...
            <div
                ...
                <p>
                    Zobrazeno
                    <span
                        class="font-bold text-gray-900">{{ $logs->firstItem() ?? 0 }}–{{ $logs->lastItem() ?? 0 }}</span>
