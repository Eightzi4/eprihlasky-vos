...
@section('form-content')
    ...
    <div x-data="stepValidator({
        ...
        <x-form-section title="Předchozí vzdělání" description="Vyplňte údaje o střední škole a nahrajte maturitní vysvědčení.">
            <x-form-field name="previous_school" label="Název střední školy" icon="school" :span="2"
                ...
                placeholder="Např. Obchodní akademie Uherské Hradiště" :locked="$isLocked" />
            <x-form-field name="izo" label="IZO školy" icon="pin"
                :value="old('izo', $application->izo)" placeholder="60371731" :locked="$isLocked" />
            <x-form-field name="school_type" label="Typ školy" icon="apartment" type="select"
                :options="['' => 'Vyberte typ', 'GYM' => 'Gymnázium', 'SOŠ' => 'SOŠ (Střední odborná škola)', 'SOU' => 'SOU (Střední odborné učiliště)', 'Jiné' => 'Jiné']"
                :value="old('school_type', $application->school_type)" :locked="$isLocked" />
            <x-form-field name="previous_study_field" label="Obor studia" icon="menu_book"
                :value="old('previous_study_field', $application->previous_study_field)"
                placeholder="Např. Ekonomické lyceum" :locked="$isLocked" />
            <x-form-field name="previous_study_field_code" label="Kód oboru (KKOV)" icon="tag"
                :value="old('previous_study_field_code', $application->previous_study_field_code)"
                placeholder="18-20-M/01" :locked="$isLocked" />
            <x-form-field name="graduation_year" label="Rok maturity" icon="calendar_month"
                :value="old('graduation_year', $application->graduation_year)" placeholder="2025"
                :locked="$isLocked" />
            <x-form-field name="grade_average" label="Průměr známek" icon="functions"
        ...
        <x-step-footer :application="$application"
            prev-route="application.step1" prev-label="Zpět na osobní údaje"
